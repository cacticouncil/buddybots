...
void idForce_Drag::Evaluate( int time ) {
	...
	physics->SetAngularVelocity( rotation.ToAngularVelocity() / MS2SEC( ( USERCMD_MSEC * tickRate ) ), id );

	velocity = physics->GetLinearVelocity( id ) * damping + dir1 * ( ( l1 - l2 ) * ( 1.0f - damping ) / MS2SEC( ( USERCMD_MSEC * tickRate ) ) );
