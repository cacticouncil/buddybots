...
void idSessionLocal::HandleMainMenuCommands(const char *menuCommand) {
	...
	for (icmd = 0; icmd < args.Argc(); ) {
		...
		if (!idStr::Icmp(cmd, "BOTScan")) {
			// file read here
			//void * guiReadPointer = &guiFileReadString;
			fileSystem->ReadFile(va("loadedBots/%s", guiFileRead), (void **)&guiFileChar, NULL);

			idStr guiFileReadString((const char *)guiFileChar);

			int loop, botCount;

			guiMainMenu_BotList->Clear();
			guiMainMenu_BotList->SetSelection(-1);


			botCount = 0;

			//while (guiFileReadString[loop] != '\0') {
			//	if (guiFileReadString[loop] == '\n') {
			//		botCount++;
			//	}
			//	++loop;
			//}

			for (loop = 0; loop < guiFileReadString.LengthWithoutColors(); ++loop) {
				if ('\r' == guiFileReadString[loop]) {
					if ('\n' == guiFileReadString[++loop]) {
						++botCount;
					}
				}
			}
			...
			for (arrayLoop = 0; arrayLoop < botCount; ++arrayLoop) {

				for (; loop < guiFileReadString.LengthWithoutColors(); ++loop) {
					if (guiFileReadString[loop] == '\r') {
						++loop;
						if (guiFileReadString[loop] == '\n') {
							++loop;
							break;
					...
					tempString += guiFileReadString[loop];
				}
					guiBotNames.Insert(tempString , arrayLoop);
					tempString.Clear();
			}
		...
		if (!idStr::Icmp(cmd, "QUEUEDBotScan")) {
			...
			num = 0; // however many bots we have

			for (i = 0; i < num; ++i) {
				// guiMainMenu_BotList->Add(i, );
			}
			continue;
		}
