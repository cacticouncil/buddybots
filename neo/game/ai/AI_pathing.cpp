...
============
*/
/*Curtis And Jesse edited to replace idqueue with stdqueue*/
pathNode_t *BuildPathTree(const obstacle_t *obstacles, int numObstacles, const idBounds &clipBounds, const idVec2 &startPos, const idVec2 &seekPos, obstaclePath_t &path) {
	...
	std::queue<pathNode_t *> pathNodeQueue, treeQueue;
	...
	root->numNodes = 0;
	pathNodeQueue.push(root);
	//pathNodeQueue.Add( root );

	for ( /*node = pathNodeQueue.Get()*/node = pathNodeQueue.front(), pathNodeQueue.pop(); node && pathNodeAllocator.GetAllocCount() < MAX_PATH_NODES; /*node = pathNodeQueue.Get()*/ node = pathNodeQueue.front() /*, pathNodeQueue.pop()*/) {
		//treeQueue.Add( node );
		if (!pathNodeQueue.empty()) { pathNodeQueue.pop(); }
		treeQueue.push(node);
